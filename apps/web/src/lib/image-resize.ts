...
async function generateThumbnailWithWorker(
  ...
  const result = new Promise((resolve, reject) => {
    console.time("generateThumbnailWithWorker");
    ...
    console.timeEnd("generateThumbnailWithWorker");
  });

  try {
    return (await result) as string;
  } catch (error) {
    console.warn("Failed to get thumbnail from worker", error);
    try {
      const thumb = await exifr.thumbnailUrl(file);
      if (thumb) {
        return thumb;
      }
    } catch (error) {
      console.warn("Failed to get thumbnail from exifr", error);
      throw error;
    }
  }
  throw new Error("Failed to get thumbnail from worker");
}
...
async function generateThumbnailMainThread(
  ...
  try {
    ...
    console.warn("Failed to get thumbnail from main thread", error);
    try {
      const thumb = await exifr.thumbnailUrl(file);
      if (thumb) {
        return thumb;

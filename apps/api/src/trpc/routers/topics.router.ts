...
export const topicsRouter = createTRPCRouter({
  getByMarathonId: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      return await getTopicsByMarathonIdQuery(ctx.db, {
        id: input.id,
      });
    }),
  getByDomain: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      return await getTopicsByDomainQuery(ctx.db, {
        domain: input.domain,
      });
    }),
  ...
  getPublicByDomain: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      return (
        await getTopicsByDomainQuery(ctx.db, {
          domain: input.domain,
        })
      ).map(hideNonPublicTopic);
    }),
  getById: publicProcedure
    .input(getTopicByIdSchema)
    .query(async ({ ctx, input }) => {
      return await getTopicByIdQuery(ctx.db, {
        id: input.id,
      });
    }),
  update: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (data.orderIndex !== undefined) {
        const currentTopic = await getTopicByIdQuery(ctx.db, { id: input.id });
        if (!currentTopic) {
          throw new TRPCError({
            code: "NOT_FOUND",
        ...
        const result = await updateTopicQuery(ctx.db, { id: inputId, data });
      ...
      return await updateTopicQuery(ctx.db, { id: inputId, data });
    }),
  updateOrder: publicProcedure
    .input(updateTopicsOrderSchema)
    .mutation(async ({ ctx, input }) => {
      return await updateTopicsOrder(ctx.supabase, {
  ...
  getWithSubmissionCount: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      return await getTopicsWithSubmissionCountQuery(ctx.db, {
        domain: input.domain,
      });
    }),
  ...
  getTotalSubmissionCount: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      return await getTotalSubmissionCountQuery(ctx.db, {
        marathonId: input.marathonId,
      });
    }),
  ...
  getScheduled: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      return await getScheduledTopicsQuery(ctx.db);
    }),
});

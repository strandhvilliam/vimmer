...
async function exportParticipantSubmissionsToZip({
  ...
  try {
    ...
    let zippedSubmission = participantData.zippedSubmissions[0]

    if (!zippedSubmission) {
      zippedSubmission =
        (await createZippedSubmissionMutation(db, {
          data: {
...
function validateEventPayload(payload: EventPayload): ExportParticipantConfig {
  ...
  if (!participantReference) throw new Error("participantReference is required")

...
export const handler = async (event: SQSEvent): Promise<void> => {
  try {
    ...
    await Promise.all(
      keys.map((key) => processSubmission(key, s3Client, realtimeClient)),
    );
  } catch (error) {
    console.error(error);
  ...
  try {
    await client.send(
      new PublishCommand({
        ...
        topic: `${Resource.App.name}/${Resource.App.stage}/submissions-status/${participantId}`,
      }),
    );
  } catch (error) {
    console.error(
      "Error publishing to realtime " + key + " " + participantId,
      error,
    );
  }
}
...
async function processSubmission(
  key: string,
  s3Client: S3Client,
  realtimeClient: IoTDataPlaneClient,
) {
  let errorParticipantId: number | undefined;
  let errorSubmissionId: number | undefined;
  try {
    ...
    errorParticipantId = participant.id;
    ...
    errorSubmissionId = submission.id;
    const exif =
    ...
    await publishToRealtime(realtimeClient, {
      key,
